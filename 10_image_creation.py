...
Batch processing for all available water years.
Production-grade implementation showing raw 4km grid blocks with a smooth colorbar across CONUS.
"""
...
# =====================================================================
# GEOSPATIAL BASEMAP INITIALIZATION (US & STATES)
# =====================================================================
print("Loading US State boundaries for basemap...")
try:
    # Fetch low-res state boundaries for a clean background overlay
    ...
    # Filter for Continental US states to optimize rendering
    us_states = us_states[us_states['iso_a2'] == 'US']
    if us_states.crs != "EPSG:4326":
        us_states = us_states.to_crs(epsg=4326)
    print("✓ Successfully loaded US base geography.")
except Exception as e:
    print(f"⚠️ Warning: Could not load live basemap geometries ({e}). falling back to local drawing.")
    us_states = None
...
# COMMENTED OUT: Durham and NC Shapefiles for global CONUS focus
...
for nc_file in nc_files:
    ...
    try:
        ...
        # REORIENTED: Adjusted bounds to completely cover the Continental US (CONUS)
        crop_lat_min = 23.0
        crop_lat_max = 50.0
        crop_col_min = -126.0
        crop_col_max = -66.0
        ...
        # =====================================================================
        fig, ax = plt.subplots(figsize=(12, 7.5)) # Slightly adjusted ratio for ideal CONUS layout
        ax.set_aspect('equal')

        # Preserves the raw 4km resolution cells on the map
        im = ax.pcolormesh(
        ...
        ax.set_title(
            ...
            f'CONUS404 WRF Downscaling Model — Continental United States (CONUS)', 
            fontsize=12, 
            fontweight='bold', 
            pad=16, 
        ...
        # COMMENTED OUT: Boundary layers are suppressed for full CONUS visibility
        ...
        #     )
# NEW: Overlay US State outlines cleanly over the pcolormesh cells
        if us_states is not None:
            us_states.plot(
                ...
                linewidth=0.75, 
                alpha=0.40,
                zorder=2  # Ensures boundaries display crisply over grid data cells
            )
        ...
        output_filename = os.path.join(output_dir, f'max_wind_speed_conus_{year_str}.png')
